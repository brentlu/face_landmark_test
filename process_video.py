...
class FacialEngine:
    ...
    def process_one_video(self, options):
        input_csv_path = ''

        # first 64KB should be sufficient
        file_hash = self.calculate_md5_digest(64 * 1024)
        ...
        self.logger.print('Check process parameters:')
        ...
        self.logger.print('  output csv path   = %s' % (output_csv_path))
        ...
        if options['output_video'] != False:
            ...
            self.logger.print('  output video path = %s' % (output_video_path))
        ...
        ret = self.process_one_video_internal(input_csv_path, options)
        if ret != False and \
           options['output_video'] != False and options['compress_video'] != False:
            ret = self.compress_one_video(output_video_path)
    ...
    def process_one_video_internal(self, input_csv_path, options):
        # init for video
        csv_index = 0
        frame_index = 0
        frame_fail_count = 0
        ...
        while True:
            ...
            ret = self.process_one_frame(frame, frame_result, options)
            if ret == False:
                frame_fail_count += 1
            else:
    ...
    def process_one_frame(self, frame, frame_result, options):
        # init for frame
        state = 'init'
        ...
        while True:
            if state == 'init':
                ...
                if faces_num == 0:
                    self.logger.print('process_one_frame: no face found by hog detector')
                ...
                if faces_num == 0:
                    self.logger.print('process_one_frame: no face found by cnn detector')
                    return False
                ...
                if target == None:
                    ...
                    self.logger.print('process_one_frame: fail to find target face')
                ...
                self.logger.print('process_one_frame: success', end = '\r')
                return True
            else:
                self.logger.print('process_one_frame: unknown state %s' % (state))
                return False
    ...
    def compress_one_video(self, video_path):
    ...
    def get_csv_data_file(self):
        ...
        ret = self.process_one_video(options)
...
def main():
    ...
    if os.path.isfile(input_video_path) != False:
        ...
        ret = engine.process_one_video(options)
